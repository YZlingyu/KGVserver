...
public class CharController {
    ...
    public DataResponse saveReport(Report report){
        try {
            report.setLayouts("["+report.getLayouts() + "]");
    ...
    public DataResponse saveKnowledgeGroup(KnowledgeGroup knowledgeGroup) {
        try {
            long id = chartService.saveKnowledgeGroup(knowledgeGroup);
            return new DataResponse("success", "200", id);
        } catch (Exception e) {
     ...
     * @Date: 16:41 2018/9/14
     */
//    @RequestMapping(value = "addKnowledgeGroup", method = RequestMethod.POST)
//    public DataResponse addKnowledgeGroup(KnowledgeGroup knowledgeGroup) {
//        try {
//            Boolean result = chartService.addKnowledgeGroup(knowledgeGroup);
//            return new DataResponse("success", "200", result);
//        } catch (Exception e) {
//            logger.error("failed to CharController.addKnowledgeGroup", e);
//            return DataResponse.buildErrorResponse();
//        }
//    }

    @RequestMapping(value = "selectKnowledgeGroup/{id}", method = RequestMethod.GET)
    public DataResponse selectKnowledgeGroup(@PathVariable("id") long id){
        Map<String, Set> result= new HashMap<String, Set>();
        Set<Technology> technologySet = new LinkedHashSet<Technology>();
        Set<Map<String,Integer>> Relations =  new HashSet<Map<String,Integer>>();
        Technology root = rTechnologyTechnologyRepository.findTechnologyById(id);
        technologySet.add(root);
        ...
        result.put("nodes", technologySet);
        ...
        try {
            ...
            for (Map.Entry entry : techs.entrySet()) {
                ...
                for (Map.Entry entry1 : techs1.entrySet()) {
                    j++;
                    list.add(new Tech((Long) entry1.getKey(),(String) entry1.getValue(),3));
...
//    public DataResponse addSonNode(Long id,String name){
//        List<Technology> list = rTechnologyTechnologyRepository.findChildrens2(id);
//
//    }
}
